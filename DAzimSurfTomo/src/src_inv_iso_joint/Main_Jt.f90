...
program SurfAniso
        ...
        real,dimension(:,:),allocatable:: pvallL,distL
      	real sta1_lat,sta1_lon,sta2_lat,sta2_lon
        real,dimension(:,:),allocatable:: obstL
        real dist1
        integer,dimension(:),allocatable:: dallL
        integer dall
       	integer istep
       	real,parameter :: pi=3.1415926535898
       	integer checkstat
       	integer ii,jj,kk
        real, dimension (:,:,:), allocatable :: scxfL,sczfL
        real, dimension (:,:,:,:), allocatable :: rcxfL,rczfL
        ...
        real,dimension(:,:,:),allocatable::vsf
      	...
      	real velvalue
      	integer,dimension(:), allocatable:: knumL,knumoL,istep1L,istep2L,istepL
        ...
        write(6,*)'allocating L', nBatch
        allocate(scxfL(nBatch,nsrc,kmax),sczfL(nBatch,nsrc,kmax),&
        rcxfL(nBatch,nrc,nsrc,kmax),rczfL(nBatch,nrc,nsrc,kmax),stat=checkstat)
        write(6,*)'allocating L done', nBatch
        if(checkstat > 0)  write(6,*)'scxf error with allocate','nsrc=',nsrc,'kmax=',kmax
        write(6,*)'periods', nBatch
        allocate(periods(nsrc,kmax),wavetype(nsrc,kmax),nrc1(nsrc,kmax),nsrc1(kmax),&
        igrt(nsrc,kmax),stat=checkstat)
        allocate(knumL(nBatch),knumoL(nBatch),istep1L(nBatch),istep2L(nBatch),istepL(nBatch),dallL(nBatch))
        write(6,*)'periodsL', nBatch
        allocate(periodsL(nBatch,nsrc,kmax),wavetypeL(nBatch,nsrc,kmax),nrc1L(nBatch,nsrc,kmax),nsrc1L(nBatch,kmax),&
        igrtL(nBatch,nsrc,kmax),stat=checkstat)
        if(checkstat > 0) write(6,*)'error with allocate'
        write(6,*)'obst', nBatch
        allocate(obst(nrc*nsrc*kmax),dist(nrc*nsrc*kmax),stat=checkstat)
        write(6,*)'obstL', nBatch
        allocate(obstL(nBatch,nrc*nsrc*kmax),distL(nBatch,nrc*nsrc*kmax),stat=checkstat)
        obst=0
        dist=0
        if(checkstat > 0)  write(6,*)'error with allocate'
        write(6,*)'pvall', nBatch
        allocate(pvall(nrc*nsrc*kmax),stat=checkstat)
        write(6,*)'pvallL', nBatch
        allocate(pvallL(nBatch,nrc*nsrc*kmax),stat=checkstat)
        if(checkstat > 0)    write(6,*)'error with allocate'
        write(6,*)'pvallL done', nBatch
        istepL=0
        write(6,*)'1/4', nBatch
        istep2L=0
        write(6,*)'1/4', nBatch
        ...
        do
            ...
            if(err.eq.0) then
                if(line(1:1).eq.'#') then
                   ...
                   if(knum.ne.knumoL(count)) then
                       istepL(count)=0
                       istep2L(count)=istep2L(count)+1
                   endif
                   istepL(count)=istepL(count)+1
                   istep1L(count)=0
                   if (sta1_lat.eq. 0) then
                       write(6,*) sta1_lat
                   endif
                   sta1_lat=(90.0-sta1_lat)*pi/180.0
                   sta1_lon=sta1_lon*pi/180.0
                   scxfL(count,istepL(count),knum)=sta1_lat
                   sczfL(count,istepL(count),knum)=sta1_lon
                   periodsL(count,istepL(count),knum)=period
                   wavetypeL(count,istepL(count),knum)=wavetp
                   igrtL(count,istepL(count),knum)=veltp
                   nsrc1L(count,knum)=istepL(count)
                   knumoL(count)=knum
                else
                   read(line,*) sta2_lat,sta2_lon,velvalue
                   istep1L(count)=istep1L(count)+1
                   dallL(count)=dallL(count)+1
                   sta2_lat=(90.0-sta2_lat)*pi/180.0
                   sta2_lon=sta2_lon*pi/180.0
                   rcxfL(count,istep1L(count),istepL(count),knum)=sta2_lat
                   rczfL(count,istep1L(count),istepL(count),knum)=sta2_lon
                   call delsph(sta1_lat,sta1_lon,sta2_lat,sta2_lon,dist1)
                   distL(count,dallL(count))=dist1
                   obstL(count,dallL(count))=dist1/velvalue
                   pvallL(count,dallL(count))=velvalue
                   nrc1L(count,istepL(count),knum)=istep1L(count)
                endif
            else
                exit
            endif
        enddo
        close(87)
        write(*,'(a,i7)') ' Number of all measurements', sum(dallL)
        !----------------------------------------------------------------------!
        !  Initialization
        ...
        dallM=MaxVal(dallL)
        maxnar = spfra*dallM*nx*ny*nz*3 !sparsity fraction
        ...
        allocate(cbst(dallM+maxm*3), dsyn(dallM), stat=checkstat)
        allocate(dsyn(dallM), stat=checkstat)
        allocate(sigmaT(dallM), Tref(dallM), datweight(dallM), stat=checkstat)
        allocate(Tdata(dallM), resbst(dallM), fwdT(dallM), fwdTvs(dallM), fwdTaa(dallM), RefTaa(dallM), stat=checkstat)
        allocate(resbst_iso(dallM), stat=checkstat)
        allocate(resSigma(dallM), stat=checkstat)
        allocate(Lsen_Gsc(nx*ny,kmaxRc,nz-1), stat=checkstat)
        allocate(GVs(dallM,maxvp), GGc(dallM,maxvp), GGs(dallM,maxvp), stat=checkstat)
        ...
        deallocate(distL)
        deallocate(pvallL)
        deallocate(scxfL,sczfL)
        deallocate(rcxfL,rczfL)
        deallocate(wavetypeL,igrtL,nrc1L)
        deallocate(nsrc1L,periodsL)
        deallocate(gcf,gsf)
        if(kmaxRc.gt.0) then
            deallocate(tRc)
        endif
