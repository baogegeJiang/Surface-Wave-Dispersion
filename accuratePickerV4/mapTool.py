...
def plotTopo(m,R,topo='Ordos.grd',laN=800,loN=800,cptfile='include/wiki-2.0.cpt'):#'cpt17.txt'):
...
def plotQuakeDis(quakeLs,output='quakeDis.png',cmd='.b',markersize=0.8,\
    ...
    if isBox:
        ...
        plotOnMap(m,laLB,loLB,'g',linewidth=3,markersize=3)
    hQ,=plotOnMap(m,la,lo,cmd,markersize,alpha)
    if len(laLN)>0:
        hN,=plotOnMap(m,laLN,loLN,'or',markersize=3,mfc=[1,0,0])
    ...
    if len(laL0)>1:
        ...
        if len(laLN)==0:
            ...
            plt.legend((hQ,hC,hN,hS,hF),('Quakes','Catalog detected','Catalog not detected',\
                'Station','Faults'),\
                bbox_to_anchor=(1, 1),loc='lower right')
    else:
    ...
    plt.savefig(output.split('.')[0]+'.tiff',dpi=500)
    plt.savefig(output)
