...
def getDataDis(staInfos,name,startDate=UTCDateTime(2014,1,1),\
    endDate=UTCDateTime(2015,10,1)):
...
class Catalog(list):
    ...
    def __init__(self,L,getXY=None):
        ...
        with open(filename,'r') as f:
            ...
            for line in f.readlines():
                ...
                if tmp[-2]=='noise' or (tmp[7]=='manual' \
                    and tmp[11]=='manual'):
                    catalog.append(Catalog(tmp,getXYFromSTEAD))
            return catalog,d
    if mod=='hinet':
        catalog=[]
        with open(filename,'r') as f:
